...
const SignUpPage = () => {
  ...
  const schema = yup.object({
    ...
    email: yup
      ...
      password:yup.string().min(8,"Password must be 8 character ").required("Please enter your password")
  });
  const {
    control,
    handleSubmit,
    ...
    <LayoutAuthentication heading="SignUp">
      ...
      <Link  className=" inline-block border border-solid border-strock dark:border-darkStroke mt-6 lg:mt-3 w-full rounded-lg mx-auto">
        <div className="flex gap-2  my-3 mx-auto w-fit ">
          ...
          />
          <span className="font-medium text-base dark:text-white">Sign up with google</span>
        </div>
      </Link>
      ...
      <form onSubmit={handleSubmit(handleSingup)}>
        <Label
          type={"text"}
          placeholder={errors.fullName?.message?errors.fullName?.message:"Jone Doe"}
          name={"fullName"}
          label={"Full Name *"}
          control={control}
          className={errors.fullName?.message?"border-error placeholder:text-error":""}
        ></Label>
        <Label
          type={"text"}
          placeholder={errors.email?.message?errors.email?.message:"example@gmail.com"}
          name={"email"}
          label={"Email *"}
          control={control}
          className={errors.email?.message?"border-error placeholder:text-error":""}
        ></Label>
        <Label
          type={"password"}
          name={"password"}
          label={"Password *"}
          placeholder={errors.password?.message?errors.password?.message:"Create a password"}
          control={control}
